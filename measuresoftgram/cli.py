...
def validate_checkBox_input(selected):
    if selected > 0:
        return True
    else:
        return False
...
def define_characteristic(available_pre_config):
    ...
    while True:
        ...
        if validate_checkBox_input(len(user_characteristics["characteristics"])):
            break
        else:
    ...
    for x in user_sub_characteristics:
        if len(available_pre_config["subcharacteristics"][x]["measures"]) == 1:
            ...
            while True:
                ...
                if validate_checkBox_input(len(local_selected_measures)):
                    break
                else:
                    print(VALID_CHECKBOX_ERROR)
