...
def save_sonar_metrics(tag):
    response = requests.get(
        f'{BASE_URL_SONAR}{REPO}&metricKeys={",".join(METRICS_SONAR)}&ps=500'
    )
...
def filter_request_per_date(data, date):
    ...
    for i in data["workflow_runs"]:
        if (
            datetime.strptime(i["created_at"][:10], "%Y-%m-%d").strftime("%Y-%m-%d")
            == date
        ):
...
def save_github_metrics_issues():
    ...
    while True:
        response = requests.get(
            api_url_issues, params={"state": "all", "per_page": 100, "page": page}
        )

        if response.status_code != 200:
            print(
                f"Erro na API do GitHub (status {response.status_code}): {response.json()}"
            )
            break
